...
from keras.layers import Embedding,LSTM,TimeDistributed,Activation,Dense
    ...
    def on_epoch_end(self, epoch, logs={}):        
        ...
        for i in range(606):
            for m in range(len(y_pred[i])):
                if (self.y)[i][m] == 1 and y_pred[i][m] == 1:
                    if m == len(y_pred[i])-1:
                        j = j + 1
                    elif (self.y)[i][m+1] == 0 and y_pred[i][m+1] == 0:
                        j = j + 1
                    elif (self.y)[i][m+1] == 2 and y_pred[i][m+1] == 2:
                        if m == len(y_pred[i])-2:
                            j = j + 1
                        elif (self.y)[i][m+2] == 0 and y_pred[i][m+2] == 0:
                            j = j + 1
                        elif (self.y)[i][m+2] == 2 and y_pred[i][m+2] == 2:
                            if m == len(y_pred[i])-3:
                                j = j + 1
                            elif (self.y)[i][m+3] == 0 and y_pred[i][m+3] == 0:
                                j = j + 1
                            elif (self.y)[i][m+3] == 2 and y_pred[i][m+3] == 2:
                                j = j + 1
        train_recall = j/1134
        n=0
        for i in range(606):
        ...
        for i in range(606):
            for m in range(len(y_pred_val[i])):
                if (self.y_val)[i][m] == 1 and y_pred_val[i][m] == 1:
                    if m == len(y_pred_val[i])-1:
                        j = j + 1
                    elif (self.y_val)[i][m+1] == 0 and y_pred_val[i][m+1] == 0:
                        j = j + 1
                    elif (self.y_val)[i][m+1] ==2 and y_pred_val[i][m+1] == 2:
                        if m == len(y_pred_val[i])-2:
                            j = j + 1
                        elif (self.y_val)[i][m+2] == 0 and y_pred_val[i][m+2] == 0:
                            j = j + 1
                        elif (self.y_val)[i][m+2] == 2 and y_pred_val[i][m+2] == 2:
                            if m == len(y_pred_val[i])-3:
                                j = j + 1
                            elif (self.y_val)[i][m+3] == 0 and y_pred_val[i][m+3] == 0:
                                j = j + 1
                            elif (self.y_val)[i][m+3] == 2 and y_pred_val[i][m+3] == 2:
                                j = j + 1
        test_recall = j/1134
        n=0
        for i in range(606):
...
model.add(Embedding(5347, 300, dropout=0.2, input_length=70, mask_zero=True))
...
model.fit(train_x, label_train__y, batch_size=32, epochs=20,validation_data=(test_x,label_test__y),callbacks=[rate_callback(training_data=[train_x, train_y], validation_data=[test_x, test_y])])
